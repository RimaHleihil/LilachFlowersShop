...
public class OrderController implements Initializable{
    ...
    @FXML
    private Button GetPriceB;

    @FXML
    private Button OrderB;

    @FXML
    private CheckBox yesship;

    @FXML
    private CheckBox noship;

    @FXML
    private CheckBox yescard;

    @FXML
    private CheckBox nocard;
    ...
    void Order(ActionEvent event) throws InterruptedException {
        ...
        if (CVVTF.getText().equals("") || CardNumberTF.getText().equals("")) {
            construction+= "please fill your card details for payment.";
        }
        instructions.setText(construction);
        if(construction.equals("")){
            double price=GetPrice();
            if(yesship.isSelected())price+=20;
         //   Order_1 order=new Order_1(date.getValue(),yesship.isSelected(),Time.getText(),yescard.isSelected(),Client.getClient().getUser());
          //  order.setTotalCost(price);
         //   Message OrderMsg=new Message(Message.SaveOrder_S,order);
          //  Client.getClient().sendMessageToServer(OrderMsg);
        }
    }
